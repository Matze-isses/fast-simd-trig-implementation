...
{
    arb_t arb_x, arb_x_2, pi_4, tmp, true_result;
    arb_init(arb_x);
    arb_init(arb_x_2);
    arb_init(pi_4);
    arb_init(tmp);
    ...
    // --- NEW: res = pi/4 - x/2 computed with Arb ---
    arb_const_pi(tmp, PRECISION);          // tmp = pi
    arb_mul_2exp_si(pi_4, tmp, -2);        // pi_4 = pi / 4

    arb_mul_2exp_si(arb_x_2, arb_x, -1);   // arb_x_2 = x / 2
    arb_sub(tmp, pi_4, arb_x_2, PRECISION);// tmp = pi/4 - x/2

    double res = arf_get_d(arb_midref(tmp), ARF_RND_NEAR);
...
void compare_results_one_minus_x2_ulp_err_signed(double *x, double *y, int64_t *ulp_err, size_t n) {
    if (!ulp_err) return;

    arb_t ax, t, one, true_result;
    arb_init(ax);
    arb_init(t);
    arb_init(one);
    arb_init(true_result);
    ...
    for (size_t i = 0; i < n; i++) {
        // Handle special x consistently
        if (isnan(x[i])) {
            ulp_err[i] = ulp_error_signed_double(y[i], NAN);
            continue;
        }
        if (isinf(x[i])) {
            ulp_err[i] = ulp_error_signed_double(y[i], -INFINITY); // 1 - inf^2 = -inf
            continue;
        }
        ...
        // true_result = 1 - x^2
        arb_mul(t, ax, ax, PRECISION);
        arb_sub(true_result, one, t, PRECISION);
