...
int main(int argc, char* argv[]) {
    ...
    int num_polynomial_degree = argc - 1;
    double *polynomial_coefficiants = malloc(num_polynomial_degree * sizeof(double));
    ...
    for (int i = 0; i < num_polynomial_degree; i++) {
        polynomial_coefficiants[i] = strtod(argv[i + 1], NULL);
    }
    ...
    calculate_polynomial_values(result, values, N, polynomial_coefficiants, num_polynomial_degree);

    long double mae = 0.0L;
    mae = average_abs_deviation_ld_d(correct_values, result, N);

    free(result);
    free(values);
    free(correct_values);
    free(polynomial_coefficiants);
    return mae;
}
