...
TOP_NUM_PARAMS = 20
BOT_NUM_PARAMS = 20
TOP_NUM_CENTERS = TOP_NUM_PARAMS
BOT_NUM_CENTERS = BOT_NUM_PARAMS
...
def init_centers(num_centers: int):
    ...
    return tf.linspace(tf.cast(X_MIN, DTYPE), tf.cast(X_MAX, DTYPE), num_centers)
...
# ======================================================
# Polynomial Evaluators (a_k * (x^k - c_k))
# ======================================================
@tf.function(jit_compile=True)
def eval_poly_top(a, c, x_):
    # number of coefficients
    n = tf.shape(a)[0]
    # generate powers 0..n-1 as a Tensor
    powers = tf.cast(tf.range(n), DTYPE)
    # compute all x^k at once: shape [n, len(x_)]
    x_pows = tf.pow(x_[None, :], powers[:, None])
    # each term: a_k * (x^k - c_k)
    terms = a[:, None] * (x_pows - c[:, None])
    return tf.reduce_sum(terms, axis=0)

@tf.function(jit_compile=True)
def eval_poly_bot(a, c, x_):
    n = tf.shape(a)[0]
    powers = tf.cast(tf.range(n), DTYPE)
    x_pows = tf.pow(x_[None, :], powers[:, None])
    terms = a[:, None] * (x_pows - c[:, None])
...
def objective(top_param, bot_param, top_centers, bot_centers):
    y1 = eval_poly_top(top_param, top_centers, x)
    y2 = eval_poly_bot(bot_param, bot_centers, x)
...
ax_f.set_title(f"Approx vs tan(x) — top deg {TOP_NUM_PARAMS-1}, bottom deg {BOT_NUM_PARAMS-1}")
...
def update_plot(step, main_loss_val, loss_hist_steps, loss_hist_vals):
    ...
    fig.suptitle(
        ...
        f"top deg {TOP_NUM_PARAMS-1}, bottom deg {BOT_NUM_PARAMS-1}",
        fontsize=12
    )
