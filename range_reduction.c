...
int get_quadrant(double x) {
  ...
  if (x < END_FRIENDLY_RANGE) {
    reduced_range = fmod(x, 2.0 * M_PI);
  }
  if (END_FRIENDLY_RANGE <= x && x < pow(2, 63) - 1) {
    reduced_range = brisebarre_range_reduction(x);
    printf("Using Brisebarre! Reduced Range is %.17g\n", reduced_range);
  }
